...
function App() {
  ...
  return (
    ...
    <nav className="w-full z-50 fixed inset-x-0 top-2 flex justify-center items-center px-6">
      ...
      <p className="text-lg text-slate-200 font-medium py-1">Adegbite Raheem</p>
      <div className=" hidden md:flex gap-6 ml-6 flex-1">
      <a href="#home" className=" text-base text-textBase py-1 font-medium hover:text-slate-100 cursor-pointer duration-100 ease-in-out" > Home</a>
      <a href="#about" className=" text-base text-textBase py-1 font-medium hover:text-slate-100 cursor-pointer duration-100 ease-in-out" > About</a>
      ...
      <a href="#home" className=" text-base text-textBase py-1 font-medium hover:text-slate-100 cursor-pointer duration-100 ease-in-out" onClick={()=>setisActive(false)}> Home</a>
      <a href="#about" className=" text-base text-textBase py-1 font-medium hover:text-slate-100 cursor-pointer duration-100 ease-in-out"onClick={()=>setisActive(false)} > About</a>
      <a href="#project" className=" text-base text-textBase py-1 font-medium hover:text-slate-100 cursor-pointer duration-100 ease-in-out" onClick={()=>setisActive(false)} > Projects</a>
      <a href="#skills" className=" text-base text-textBase py-1 font-medium hover:text-slate-100 cursor-pointer duration-100 ease-in-out" onClick={()=>setisActive(false)} > Skills</a>
    ...
    <img src={Raheem} alt="dd" className=" w-full h-full absolute -right-4 bottom-4 object-cover rounded-lg shadow-lg"/>
    </div>
    </div>
    <div className="w-full h-420 flex flex-col items-center justify-center">

    <p class="text-lg text-textBase text-center">I am <b class=" text-cyan-700" >Adegbite Raheem</b>, but you can  call me <b class=" text-cyan-700" >Rahdeg.</b> I am a <b class=" text-cyan-700">Full Stack Developer  </b>. I create interactive experiences for people using modern web technology,independent fellow who has soft spot for team/group work. Committed to high standards of service, performance and integrity. Dedicated and reliable with organized, proactive and problem-solving nature.</p>
    ...
    <p className=" text-4xl text-cyan-700 flex items-center justify-center font-bold">Skills</p>
    <section className="flex flex-wrap items-center justify-evenly my-24 gap-4 " id="skills">
    {Skills && Skills.map((data,idx)=>(
      ...
      <p className=" text-1xl text-gray-300 text-center my-3">{data.name} </p>
      <p className=" text-base text-gray-300 text-center my-2 mx-2">{data.description.length > 25 ? `${data.description.slice(0,25)}...`: data.description} </p>
      </motion.div>
    ))}
    
      
    </section>
         <p className=" text-4xl text-cyan-700 flex items-center justify-center font-bold">Experience</p>
...
>
  ...
  <p>
    ...
    <p className=" text-lg text-gray-300">Technogies
    ...
    <section id="contact" className="flex flex-col items-center justify-center w-full my-24 md:flex-wrap ">
    <p className=" text-4xl text-cyan-700 flex items-center justify-center font-bold mt-2">Follow me on</p>
    <div className="flex items-center justify-center w-full my-4 flex-wrap gap-4" >
    {SocialLinks && SocialLinks.map((data,idx)=>(
